...
class Upload {
  ...
  _start(source) {
    ...
    // First, check if the caller has supplied a manual upload size.
    // Next, we try to use the calculated size from the source object.
    // Finally, we assume the full size is not yet available and defer sending
    // the upload length to the server.
    if (this.options.uploadSize != null) {
      ...
      this._size = size;
    } else if (this._source.size !== undefined) {
      this._size = this._source.size;
    } else {
      this._uploadLengthDeferred = true;
    }
  ...
  _resumeUpload() {
    ...
    xhr.onload = () => {
      ...
      if (isNaN(length) && !this._uploadLengthDeferred) {

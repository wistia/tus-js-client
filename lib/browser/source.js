...
class StreamSource {
  ...
  slice(start, end) {
    if (start != this.bufferOffset) {
      throw new Error("requested data is not the reader's current offset");
    }

    const bytesToRead = end - start;
    return this._readUntilEnoughDataOrDone(bytesToRead);
  }

  _readUntilEnoughDataOrDone(bytesToRead) {
    if (this._done || len(this._buffer) >= bytesToRead) {
      return this._getDataFromBuffer(bytesToRead);
    }
  ...
  _getDataFromBuffer(bytesToRead) {
    if (len(this._buffer) === 0 && this._done) {
      return null;
    }
    const bufferToReturn = this._buffer.slice(0, bytesToRead);
    this._buffer = this._buffer.slice(bytesToRead);
    this._bufferOffset += len(bufferToReturn);
